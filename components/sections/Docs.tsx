...
const DOC_LINKS = [
  ...
  { icon: 'api.svg',         title: 'REST API Reference',      sub: 'Full endpoint documentation',        color: 'purple',  tag: 'v1.0' },
  { icon: 'code.svg',        title: 'JavaScript SDK',          sub: 'npm install tokko',                  color: 'orange',  tag: 'NEW' },
  { icon: 'sdk.svg',         title: 'Python SDK',              sub: 'pip install tokko',                  color: 'teal',    tag: null },
  { icon: 'team.svg',        title: 'Claude Code Integration', sub: 'MCP server setup guide',             color: 'red',     tag: 'Teams' },
]
...
const iconBg: Record<string, string> = {
  ...
  red:    'bg-accent-red/10 border border-accent-red/20',
}

const tagColor: Record<string, string> = {
  NEW:    'bg-accent/10 text-accent border-accent/20',
  Teams:  'bg-accent-purple/10 text-accent-purple border-accent-purple/20',
  'v1.0': 'bg-bg-s2 text-text-muted border-border',
}

export function Docs() {
  return (
    <section id="docs" className="py-30 border-t border-border bg-bg-surface/30">
        ...
        <div className="grid grid-cols-2 gap-20 items-start">
          <div>
            ...
            <p className="font-sans text-text-muted leading-relaxed mb-10">
              Everything you need to integrate Tokko into your workflow. REST API, SDKs, and guides for every major framework.
            </p>
            <div className="flex flex-col gap-2">
              {DOC_LINKS.map((d) => (
                <a key={d.title} href="/docs"
                  className="flex items-center gap-4 bg-bg-card border border-border rounded-xl px-5 py-4 hover:border-accent/30 hover:translate-x-1 transition-all group">
                  ...
                  {d.tag && (
                    <span className={`font-mono text-[0.6rem] font-bold px-2 py-0.5 rounded border ${tagColor[d.tag]}`}>{d.tag}</span>
                  )}
                  <span className="text-text-muted group-hover:text-accent transition-colors text-sm mr-1">→</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-bg-surface border border-border rounded-2xl overflow-hidden font-mono text-[0.78rem]">
              <div className="bg-bg-s2 px-4 py-3 border-b border-border flex items-center justify-between">
                <div className="flex gap-1">
                  {['JavaScript', 'Python', 'cURL'].map((t, i) => (
                    <div key={t} className={`px-3 py-1.5 rounded text-[0.72rem] font-semibold ${i === 0 ? 'bg-border text-text' : 'text-text-muted'}`}>{t}</div>
                  ))}
                </div>
                <span className="text-[0.65rem] text-text-muted cursor-pointer hover:text-accent transition-colors">Copy</span>
              </div>
              <div className="p-5 leading-[1.75]">
...
<span className="text-accent-purple">const</span> ps = <span className="text-accent-purple">new</span> <span className="text-accent-orange">Tokko</span>{'({'}
  apiKey: <span className="text-accent">'ps_your_api_key'</span>,
...
<span className="text-text-muted">// result.costSaved → </span><span className="text-accent-teal">0.000138</span></pre>
            ...
            <div className="bg-bg-card border border-border rounded-xl p-5">
              ...
              <pre className="font-mono text-[0.75rem] leading-[1.7] text-text-muted whitespace-pre-wrap"><span className="text-border">{'{'}</span>
  ...
  <span className="text-text">"model"</span>: <span className="text-accent">"claude-sonnet"</span>
